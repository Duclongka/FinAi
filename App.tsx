...
const App: React.FC = () => {
  ...
  const handleTransferSubmit = (e: React.FormEvent) => {
    ...
    const tx1: Transaction = { 
      id: Date.now().toString(), type: 'expense', amount: amountInVnd, 
      description: `Chuyển: ${JAR_CONFIG[transferFrom].name} ➔ ${JAR_CONFIG[transferTo].name}`, 
      jarType: transferFrom, timestamp: Date.now(), transferGroupId: transferId
    };
    const tx2: Transaction = { 
      id: (Date.now()+1).toString(), type: 'income', amount: amountInVnd, 
      description: `Nhận: ${JAR_CONFIG[transferFrom].name} ➔ ${JAR_CONFIG[transferTo].name}`, 
      jarType: transferTo, timestamp: Date.now(), transferGroupId: transferId
    };
  ...
  const handleSaveLoan = (e: React.FormEvent) => {
    ...
    if (editingLoanId) {
       ...
       if (oldLoan) {
          const oldSyntheticTx: Transaction = { id: `loan_${oldLoan.id}`, type: oldLoan.type === LoanType.BORROW ? 'income' : 'expense', amount: oldLoan.principal, description: `Đảo ngược`, jarType: oldLoan.loanJar, timestamp: Date.now(), loanId: oldLoan.id };
  ...
  const handleDeleteLoan = (loanId: string) => {
    ...
    if (!loan) return;
    const initialImpactTx: Transaction = {
      id: `rev_init_${loan.id}`,
      type: loan.type === LoanType.BORROW ? 'income' : 'expense',
      amount: loan.principal,
      description: `Reverse initial loan`,
    ...
    showToast("Đã xóa khoản vay và thu hồi số dư về hũ.");
  };
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    ...
    if (amountVnd > remainingVnd + 0.01) {
       alert(`Số tiền bạn nhập vượt quá số tiền còn lại!`);
       return;
    }
    const payTx: Transaction = {
      id: `pay_${Date.now()}`,
      type: targetLoan.type === LoanType.BORROW ? 'expense' : 'income',
      amount: amountVnd,
      description: `[${targetLoan.type === LoanType.BORROW ? 'Thanh toán nợ' : 'Thu hồi nợ'}] ${targetLoan.lenderName}`,
      jarType: targetLoan.loanJar,
  ...
  const handleDeleteEventTransaction = (eventId: string, txId: string) => {
    setEvents(prev => prev.map(ev => ev.id === eventId ? {
      ...ev,
      transactions: ev.transactions.filter(t => t.id !== txId)
    } : ev));
    showToast("Đã xóa khỏi sự kiện!");
  };
  ...
  const handlePushEventToHistory = (event: EventGroup, jarType: JarType | 'AUTO') => {
    ...
    const mainTx: Transaction = {
      id: `ev_main_${event.id}`, type: mainType, amount: netAmount, description: `[Sự kiện] ${event.name}`,
      jarType: jarType === 'AUTO' ? undefined : jarType as JarType, timestamp: Date.now()
    };
  ...
  const chartData = useMemo(() => {
    ...
    if (chartRange === 'year') {
      ...
      for (let i = count - 1; i >= 0; i--) {
        ...
        data.push({ name: d.toLocaleDateString(settings.language === 'vi' ? 'vi-VN' : 'en-US', { day: '2-digit', month: '2-digit' }), Thu: convertValue(dayT.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0)), Chi: convertValue(dayT.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0)) });
      }
    }
    return data;
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans flex flex-col items-center pt-[calc(var(--sat,0px)+120px)] pb-[calc(var(--sab,0px)+100px)]">
      {toast && (
        <div className="fixed left-1/2 -translate-x-1/2 z-[100] bg-indigo-600 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase shadow-2xl animate-in slide-in-from-top-4" style={{ top: 'calc(var(--sat, 0px) + 12px)' }}>
          {toast.msg}
        </div>
      )}
      ...
      {isAuthModalOpen && !currentUser && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-900/90 backdrop-blur-2xl p-4">
          <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-[3rem] w-full max-md:max-w-md p-10 shadow-2xl animate-in zoom-in-95">
            <div className="flex flex-col items-center text-center space-y-6">
              <h1 className="text-2xl font-black text-slate-900">{t.onboarding_welcome}</h1>
              <div className="w-full space-y-5">
                <input type="text" value={onboardingForm.name} onChange={e => setOnboardingForm({...onboardingForm, name: e.target.value})} placeholder={t.onboarding_name} className="w-full bg-white border-2 border-slate-100 rounded-[2rem] px-6 py-4 text-sm font-bold outline-none" />
                <div className="flex gap-4">
                  <button onClick={() => setOnboardingForm({...onboardingForm, gender: 'male'})} className={`flex-1 py-4 rounded-[2rem] text-xs font-black transition-all border-2 ${onboardingForm.gender === 'male' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400'}`}>{t.onboarding_male}</button>
                  <button onClick={() => setOnboardingForm({...onboardingForm, gender: 'female'})} className={`flex-1 py-4 rounded-[2rem] text-xs font-black transition-all border-2 ${onboardingForm.gender === 'female' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400'}`}>{t.onboarding_female}</button>
                </div>
              </div>
              <button onClick={() => onboardingForm.name.trim() && setCurrentUser({ id: Date.now().toString(), displayName: onboardingForm.name, gender: onboardingForm.gender, email: '', provider: 'local' })} disabled={!onboardingForm.name.trim()} className="w-full py-5 bg-blue-600 text-white rounded-[2rem] text-xs font-black uppercase shadow-xl">{t.onboarding_start}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-5xl mx-auto flex flex-col gap-1.5 p-3">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-3">
              <button onClick={() => setIsSettingsOpen(true)} className="p-2 -ml-2 text-slate-500 hover:text-indigo-600">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
              <div className="flex flex-col">
                <div className="text-[9px] font-black text-indigo-500 uppercase tracking-[0.1em]">👋 HI, <span className="text-slate-900">{currentUser?.displayName || 'YOU'}</span></div>
        ...
        {activeTab === 'home' && (
          <>
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: t.stats_jars, val: (Object.values(balances) as number[]).reduce((a, b) => a + b, 0), icon: '💰', colorClass: 'text-slate-900' }, 
          ...
          <section className="bg-white p-5 rounded-[2.5rem] border-2 border-slate-200 shadow-xl min-h-[500px]">
            ...
            <div className="space-y-2">
              {displayedTransactions.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-[10px] font-bold text-slate-400 italic bg-slate-50 rounded-3xl border-2 border-dashed border-slate-100">{t.history_empty}</div>
              ) : (
                <>
                  ...
                  {filteredTransactions.length > 5 && filteredTransactions.length > visibleTxCount && (
                    <button onClick={() => setVisibleTxCount(prev => prev + 15)} className="w-full py-4 text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:bg-indigo-50 rounded-2xl">{t.history_more}</button>
                  )}
                </>
              )}
            </div>
            ...
            <section className="bg-white p-6 rounded-[2rem] border-2 border-slate-200 shadow-xl">
              ...
              <div className="mt-6 flex items-center justify-between gap-2 pt-4 border-t border-slate-50">
                <div className="flex-1 bg-emerald-50 p-3 rounded-2xl border border-emerald-100">
                  <p className="text-[8px] font-black text-emerald-600 uppercase mb-0.5">Tổng thu</p>
                  <p className="text-[10px] font-black text-emerald-700">{formatCurrency(rangeTotalsVnd.income)}</p>
                </div>
                <div className="flex-1 bg-rose-50 p-3 rounded-2xl border border-rose-100">
                  <p className="text-[8px] font-black text-rose-600 uppercase mb-0.5">Tổng chi</p>
                  <p className="text-[10px] font-black text-rose-700">{formatCurrency(rangeTotalsVnd.expense)}</p>
                </div>
                <div className="flex-1 bg-indigo-50 p-3 rounded-2xl border border-indigo-100">
                  <p className="text-[8px] font-black text-indigo-600 uppercase mb-0.5">Tổng thu/chi</p>
                  <p className="text-[10px] font-black text-indigo-700">{rangeTotalsVnd.net >= 0 ? '+' : ''}{formatCurrency(rangeTotalsVnd.net)}</p>
                </div>
              </div>
            </section>
          ...
          <div className="space-y-8 animate-in fade-in duration-300 pb-20">
            <section className="bg-white p-6 rounded-[2rem] border-2 border-slate-200 shadow-xl">
              <div className="flex items-center justify-between mb-6 border-b pb-3">
                ...
                <button onClick={handleOpenNewLoan} className="px-4 py-2 bg-indigo-600 text-white text-[9px] font-black uppercase rounded-full shadow-lg">＋ {t.loan_new}</button>
              </div>
                  ...
                  {loans.map(loan => (
                    <div key={loan.id} onDoubleClick={() => { setSelectedLoan(loan); setIsLoanDetailModalOpen(true); }} className="bg-slate-50 rounded-2xl border border-slate-100 p-4 group relative pb-10 cursor-pointer transition-all hover:bg-white">
                      ...
                      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 flex items-center gap-2 transition-all">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); handleTripleDelete(loan.id); }} className={`w-7 h-7 rounded-full flex items-center justify-center shadow-md text-[10px] transition-all ${deleteClickData.id === loan.id ? 'bg-red-600 text-white animate-pulse' : 'bg-white text-rose-600 border border-rose-100'}`}>
                          {deleteClickData.id === loan.id ? (deleteClickData.count >= 2 ? '❓' : deleteClickData.count) : '🗑️'}
                        </button>
                      </div>
                    </div>
                  ))}
            ...
            <section className="bg-white p-6 rounded-[2rem] border-2 border-slate-200 shadow-xl">
              <div className="flex items-center justify-between mb-6 border-b pb-3">
                ...
                <button onClick={() => setIsRecurringModalOpen(true)} className="px-4 py-2 bg-emerald-600 text-white text-[9px] font-black uppercase rounded-full shadow-lg">＋ {t.recurring_add}</button>
              </div>
                    ...
                    <div key={tpl.id} onDoubleClick={() => { setSelectedRecurring(tpl); setIsRecurringDetailModalOpen(true); }} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group transition-all cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[8px] font-bold text-slate-400 uppercase">Bắt đầu: {tpl.startDate} • {tpl.subscriptionType}</p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end">
                        <div className="flex items-center gap-3">
            ...
            <section className="bg-white p-6 rounded-[2rem] border-2 border-slate-200 shadow-xl mb-10">
              <div className="flex items-center justify-between mb-6 border-b pb-3">
                ...
                <button onClick={() => setIsEventModalOpen(true)} className="px-4 py-2 bg-rose-600 text-white text-[9px] font-black uppercase rounded-full shadow-lg">{t.event_add}</button>
              </div>
                           ...
                           {filteredTxs.length === 0 ? <p className="text-center text-[9px] text-slate-300 italic py-4">Chưa có giao dịch lọc</p> : (
                             filteredTxs.map(et => (
                               <div key={et.id} className="flex justify-between items-center text-[10px] py-2 border-b border-slate-100 last:border-none">
                                 <span className="text-slate-700 font-bold">{et.description}</span>
                                 <div className="flex items-center gap-2">
                                    <span className={et.type === 'income' ? 'text-emerald-600 font-black' : 'text-rose-600 font-black'}>{formatCurrency(et.amount)}</span>
                                    <button onClick={() => handleDeleteEventTransaction(ev.id, et.id)} className="w-5 h-5 flex items-center justify-center text-rose-300 hover:text-rose-600 font-black transition-colors">✕</button>
                                 </div>
                               </div>
                             ))
                           )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-t border-indigo-100/50 p-2 shadow-2xl" style={{ paddingBottom: 'calc(var(--sab, 0px) + 12px)' }}>
        <div className="max-w-5xl mx-auto flex items-center justify-around">
          {[
            ...
            { id: 'entry', icon: '＋', special: true },
            { id: 'overview', icon: '📊', label: t.nav_overview },
            { id: 'loans', icon: '🏦', label: t.nav_loans }
          ].map(btn => btn.special ? (
            <div key={btn.id} className="relative flex items-center justify-center px-4 -mt-2">
            ...
            <button key={btn.id} onClick={() => setActiveTab(btn.id as AppTab)} className={`flex flex-col items-center gap-1 p-2 min-w-[70px] transition-all ${activeTab === btn.id ? 'text-indigo-600 scale-105' : 'text-slate-400'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              {settingsTab === 'app' && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[8px] font-normal text-slate-400 uppercase tracking-widest ml-1">{t.currency}</label>
                    <select value={settings.currency} onChange={e => setSettings({...settings, currency: e.target.value as any})} className="w-full h-12 bg-slate-50 border-2 border-slate-100 rounded-xl px-4 text-[11px] font-normal outline-none focus:border-indigo-400 appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22none%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Cpath%20d%3D%22M5%207L10%2012L15%207%22%20stroke%3D%22%2364748B%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22/%3E%3C/svg%3E')] bg-[length:20px] bg-[right_1rem_center] bg-no-repeat">
                      <option value="VND">VND</option><option value="JPY">JPY</option><option value="USD">USD</option>
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[8px] font-normal text-slate-400 uppercase tracking-widest ml-1">{t.language}</label>
                    <select value={settings.language} onChange={e => setSettings({...settings, language: e.target.value as any})} className="w-full h-12 bg-slate-50 border-2 border-slate-100 rounded-xl px-4 text-[11px] font-normal outline-none focus:border-indigo-400 appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22none%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Cpath%20d%3D%22M5%207L10%2012L15%207%22%20stroke%3D%22%2364748B%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22/%3E%3C/svg%3E')] bg-[length:20px] bg-[right_1rem_center] bg-no-repeat">
                      <option value="vi">{t.lang_vi}</option><option value="en">{t.lang_en}</option><option value="ja">{t.lang_ja}</option>
                    </select>
                  </div>
                  <div className="space-y-2">
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-sm:max-w-sm p-8 shadow-2xl animate-in zoom-in-95 border-2 border-slate-200">
             ...
             <div className="space-y-4 text-[11px] font-bold text-slate-600">
                ...
                <div className="flex justify-between border-b pb-2"><span>Số tiền:</span><span className={selectedTx.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}>{formatCurrency(selectedTx.amount)}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Hũ:</span><span className="text-indigo-600">{selectedTx.jarType ? t[`jar_${selectedTx.jarType.toLowerCase()}_name`] : 'Tự động phân bổ'}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Ngày:</span><span>{new Date(selectedTx.timestamp).toLocaleString()}</span></div>
                {selectedTx.note && <div className="flex flex-col border-b pb-2"><span>Ghi chú:</span><span className="text-slate-400 font-normal italic mt-1">{selectedTx.note}</span></div>}
             </div>
             <button onClick={() => setIsHistoryDetailModalOpen(false)} className="w-full mt-8 py-4 bg-indigo-600 text-white font-black uppercase text-[10px] rounded-2xl shadow-xl">Đóng</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-sm:max-w-sm p-8 shadow-2xl animate-in zoom-in-95 border-2 border-slate-200">
             ...
             <div className="space-y-4 text-[11px] font-bold text-slate-600">
                ...
                <div className="flex justify-between border-b pb-2"><span>Đối tác:</span><span className="text-slate-900 uppercase">{selectedLoan.lenderName}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Số tiền gốc:</span><span className="text-slate-900">{formatCurrency(selectedLoan.principal)}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Đã trả/thu:</span><span className="text-emerald-600">{formatCurrency(selectedLoan.paidAmount)}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Còn lại:</span><span className="text-rose-600">{formatCurrency(selectedLoan.principal - selectedLoan.paidAmount)}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Hũ liên quan:</span><span className="text-indigo-600 uppercase">{selectedLoan.loanJar ? t[`jar_${selectedLoan.loanJar.toLowerCase()}_name`] : 'Tự động phân bổ'}</span></div>
                <div className="flex justify-between border-b pb-2"><span>Ngày bắt đầu:</span><span>{selectedLoan.startDate}</span></div>
             </div>
             <button onClick={() => setIsLoanDetailModalOpen(false)} className="w-full mt-8 py-4 bg-indigo-600 text-white font-black uppercase text-[10px] rounded-2xl shadow-xl">Đóng</button>
          </div>
        </div>
      )}
      ...
      {isCalcOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md">
          <div className="bg-white/95 rounded-[2.5rem] w-full max-w-[320px] p-7 shadow-2xl border-2 border-slate-200">
            <div className="bg-white border-2 border-slate-100 rounded-3xl p-6 mb-6 text-right shadow-inner min-h-[100px] flex flex-col justify-end">
              <div className="text-slate-400 text-[10px] font-bold overflow-hidden text-ellipsis mb-1">{calcExpr || '0'}</div>
              <div className="text-slate-900 text-2xl font-black">{evaluateMath(calcExpr) || '0'}</div>
            </div>
            <div className="grid grid-cols-4 gap-3">
              {['(', ')', '%', '/'].map(btn => (<button key={btn} onClick={() => onCalcPress(btn)} className="h-14 rounded-2xl bg-slate-100 text-slate-700 font-black text-lg active:scale-90 transition-all">{btn}</button>))}
              {['7', '8', '9', '*'].map(btn => (<button key={btn} onClick={() => onCalcPress(btn)} className="h-14 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-lg active:scale-90 transition-all">{btn}</button>))}
              {['4', '5', '6', '-'].map(btn => (<button key={btn} onClick={() => onCalcPress(btn)} className="h-14 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-lg active:scale-90 transition-all">{btn}</button>))}
              {['1', '2', '3', '+'].map(btn => (<button key={btn} onClick={() => onCalcPress(btn)} className="h-14 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-lg active:scale-90 transition-all">{btn}</button>))}
              <button onClick={() => onCalcPress('C')} className="h-14 rounded-2xl bg-rose-50 text-rose-600 font-black text-lg active:scale-90">C</button>
              <button onClick={() => onCalcPress('0')} className="h-14 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-lg active:scale-90">0</button>
              <button onClick={() => onCalcPress('.')} className="h-14 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-lg active:scale-90">.</button>
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] w-full max-sm:max-w-xs p-8 shadow-2xl relative animate-in zoom-in-95 border-2 border-slate-200">
            ...
            <form onSubmit={handleSaveEvent} className="space-y-6">
               ...
               <div className="flex gap-3"><button type="button" onClick={() => setIsEventModalOpen(false)} className="flex-1 py-4 bg-slate-100 text-slate-400 font-black uppercase text-[10px] rounded-2xl active:scale-95">{t.manual_cancel}</button><button type="submit" className="flex-[2] py-4 bg-rose-600 text-white font-black uppercase text-[10px] rounded-2xl shadow-xl active:scale-95 transition-all">{t.onboarding_start}</button></div>
            </form>
          </div>
        </div>
      )}
